...
def show_camera():
    ...
    if cap.isOpened():
        window_handle = cv2.namedWindow("USB Camera", cv2.WINDOW_FULLSCREEN)
        # Window
...
def capture_info(capture):
    
    print("Capture width and height : {}x{}".format(capture.get(cv2.CAP_PROP_FRAME_WIDTH),
                                                    capture.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    print("Capture FPS : {}".format(capture.get(cv2.CAP_PROP_FPS)))
